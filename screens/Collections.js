    ...
    <ScrollView style={taskStyles.body}>
        <View style={collectionStyles.header}>
            ...
            <TouchableOpacity onPress={() => setShowForm(true)} style={collectionStyles.plusButton}>
                <AntDesign style={[taskStyles.head, collectionStyles.plus]} name="plus"/>
            </TouchableOpacity>
        </View>
            ...
            {tasks.map((collection, index) => {
                return(
                    <View key={index} style={collectionStyles.group}>
                        ...
                        <View style={collectionStyles.header}>
                            <TouchableOpacity onPress={() => handleEdit(index)} style={[taskStyles.sub, {paddingHorizontal: 20}]}>
                                <AntDesign style={[taskStyles.head, taskStyles.edit]} name="edit"/>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => deleteCollection(index)} style={[taskStyles.sub, {paddingHorizontal: 20}]}>
...
export default connect(mapStateToProps, {openCollection, createCollection, deleteCollection, editCollection})(Collections);
